...
from tw2.core import Widget, Param, Variable
from tw2.core import JSSource
from tw2.jquery.base import jquery_js

from .resources import jssor_slider_js
...
"""
    ...
    def _dict_to_js(self, options_dict):
        options_strs = []
        for k, v in options_dict.items():
            if isinstance(v, dict):
                v_str = self._dict_to_js(v)
            elif isinstance(v, (list, tuple)):
                v_str = "[{}]".format(", ".join(self._dict_to_js(x) for x in v))
            elif isinstance(v, bool):
                v_str = "true" if v else "false"
            elif isinstance(v, str) and not (v.startswith("$") and
                                             v.endswith("$")):
                v_str = "'{}'".format(v)
            else:
                v_str = str(v)
